...
public class No46 {
	...
	public int solution1(int[] nums) { // 1번째 방법
        int answer = 0;
        int num = 0; // 변동 값 확인
        ...
        for(int i = 0; i < nums.length; i++) {
        	if(answer == nums.length / 2) break; // 폰켓몬 종류가 길이의 / 2을 넘어설 경우 종료
        	if(num != nums[i]) { // 폰켓몬의 종류가 바뀌면 종류++
        		num = nums[i];
        		answer++;
        	}
        }
	...
	public int solution2(int[] nums) { // 2번째 방법
		HashSet<Integer> hs = new HashSet<>(); // 중복 제거를 위해 HashSet 사용
	...
	public static void main(String[] args) {
		...
		int[] numArr1 = {3, 1, 2, 3};
		int[] numArr2 = {3, 3, 3, 2, 2, 4};
		
		System.out.println(test.solution1(numArr1));
		System.out.println(test.solution1(numArr2));

		System.out.println(test.solution2(numArr1));
		System.out.println(test.solution2(numArr2));
	}
}
